...
def sflux(data, fs, winlen, ovrlen, nftt):
    ...
    #ft为 每一帧的SFT值用来和threshold来比较判断是否为pitch帧
    ft = (Num + eps) / (Den + eps)
...
def snre_highenergy(fdata, nfr10, flen, fsh10, ENERGYFLOOR, pv01, pvblk):
    ...
    Dexpl = 18;
    #left 不依赖后文信息 right 要依赖后面的信息因此改掉~
    Dexpr = 0;
    segThres = 0.25
    ...
    #the size of super-segement
    NESEG = 20
    win_shift = 5

# the energy of every frame
...
##能量计算方法有待改进
    ...
    for i in range(1,int((nfr10 - NESEG) / (win_shift) + 1)):
        eY = numpy.sort(e[range((i-1) * win_shift +1,(i-1) * win_shift +1+NESEG)])
        eY = numpy.insert(eY,0,'inf')

        emin[range((i-1) * win_shift +1, (i-1) * win_shift +1+NESEG)] = eY[int(numpy.floor(NESEG*0.1))]

        if numpy.not_equal(i,1):
            emin[range((i-1) * win_shift +1,(i-1) * win_shift +1+NESEG)] = 0.9 * emin[(i-1) * win_shift +1] +0.1 * emin[(i-1) * win_shift +1+NESEG]

    if numpy.not_equal(i * win_shift + 1,nfr10):
        eY = numpy.sort(e[range( i * win_shift + 1, nfr10)])
        eY = numpy.insert(eY, 0, 'inf')

        emin[range(i * win_shift + 1, nfr10 + 1 )] = eY[int(numpy.floor((nfr10 - ((i-1)*win_shift+1))*0.1))]
        emin[range(i * win_shift + 1, nfr10 + 1 )] = 0.9 * emin[i*win_shift] + 0.1 * emin[i*win_shift+1]
    ...
    for i in range(1, int((nfr10 - NESEG) / (win_shift) + 1)):
        Dsmth_max[range((i-1) * win_shift +1, (i-1) * win_shift +1+NESEG)] = numpy.amax(
            Dsmth[range((i-1) * win_shift +1,(i-1) * win_shift +1+NESEG)])

    if numpy.not_equal(i * win_shift + 1,nfr10):
        Dsmth_max[range(i * win_shift + 1, nfr10 + 1)] = numpy.amax(Dsmth[range((i - 1) * win_shift+ 1, nfr10 + 1)])
...
#进行一个能量的限定 设置最小能量
    ...
    nexpl = 0
    nexpr = 0  # % 29 and 39, estimated statistically, 95% ; 33, 47 %98 for voicebox pitch
    sign_vad = 0
    for i in range(1, nfr10 + 1):
    ...
    nexpl = 0;
    nexpr = 0  # ; % 9 and 13, estimated statistically 5%; 5, 12 %2 for voicebox pitch
    sign_vad = 0
    for i in range(1, nfr10 + 1):
        if (snre_vad[i] == 1) and (sign_vad == 0):
            nstart = i
...
def pitchblockdetect(pv01, pitch, nfr10, opts):
    ...
    for i in range(0, nfr10):
        ...
        if (pv01_[i] == 1) and (sign_pv == 0):
            # print("i=%s " %(i))
            nstart, sign_pv = i, 1
            pvblk[range(max([nstart - 5, 0]), nstart + 1)] = 1
            #初始值未nstart-60
            ...
            pvblk[range(nstop, numpy.amin([nstop + 5, nfr10 - 1]) + 1)] = 1
